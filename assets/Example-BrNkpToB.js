...
// Suggestions shown when adding a new key on the \`task\` object.
// Drives the "new-key" dropdown — one of the three places
// \`Select\` renders.
const newKeyOptions = ['dueDate', 'tags', 'estimate', 'reviewer']

// Standard data types plus two enums. Existing values like
// 'in-progress' and 'high' are auto-matched to their enum type
// by content (\`matchPriority\`); the type selector also lets you
// switch any value to a different enum or back to a primitive —
// exercising the other two \`Select\` call sites.
...
export default function SwapTheBuiltIns() {
  ...
  const themeName = useEditorTheme().displayName
